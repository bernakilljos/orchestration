#!/bin/bash
# common.sh - bundles_cowork 공통 헬퍼
set -uo pipefail
bundles_cowork="bundles_cowork"
REPO_ROOT="${CLAUDE_PROJECT_ROOT:-$(pwd)}"
LOG_DIR="$REPO_ROOT/.claude/state/$bundles_cowork"
DATA_DIR="$REPO_ROOT/data/$bundles_cowork/$(date +%Y-%m-%d)"
