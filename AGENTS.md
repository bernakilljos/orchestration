...
## 재발 방지 헌장 A~F (2026-08-12) — 준수 필수

Codex 도 이 헌장 준수. Codex 는 실행 담당이므로 특히 A(하드코딩)·B(검증)·D7(파일 입력) 중요.

- **A** 하드코딩·폴백 금지: 경로·%·상수 X. 정본 1곳. 기존 자산 재사용.
- **B** 검증: 검사 0건 ≠ 통과. 수정 후 자동 검증. Smoke Test 의무.
- **C** 운영 안전: 미커밋 X. 위험 명령 approval-gate. 멈춤 방지.
- **D7 파일 입력 프로토콜**: 지시에 파일 경로 포함 = 요구사항. Read 필수 (핵심 3~5줄 인용) → 체크리스트 추출 → O/X 대조표. 큰 파일도 끝까지. 못 읽으면 즉시 보고.
