...
index 4460e04..0d45926 100644
--- a/docs/ssj/render-ssj.py
+++ b/docs/ssj/render-ssj.py
@@ -5,7 +5,7 @@ import base64, pathlib
...
+ 'book':'<svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="#8b7355" stroke-width="1.5"><path d="M4 19V5a2 2 0 0 1 2-2h12a2 2 0 0 1 2 2v14H6a2 2 0 0 0 0 4h14"/></svg>',
 ...
 COMMON_CSS = """
@@ -215,56 +219,66 @@ body{{width:850px;height:1100px;background:#f5efe6;
 
 # ===================== SSJ-SUMMARY =====================
 def build_ssj_summary():
+    # 새 캐릭터 이미지 (촛불+노트북) 있으면 사용, 없으면 내캐릭터
+    laptop_img = DIR / "character-laptop.png"
+    if laptop_img.exists() and laptop_img.stat().st_size > 10000:
+        img_b64 = base64.b64encode(laptop_img.read_bytes()).decode()
+    else:
+        img_b64 = char_b64
...
+  padding:10px 14px;display:flex;flex-direction:column;overflow:hidden;gap:5px}}
...
+.header-left h1{{font-family:'Gaegu',cursive;font-size:28px;font-weight:700;color:#2c2418;line-height:1.2}}
+.header-left .sub{{font-size:11px;color:#7a6b5a;margin-top:3px;display:flex;align-items:center;gap:3px}}
+.header-right{{display:flex;gap:5px}}
+.mid{{display:grid;grid-template-columns:232px 1fr 225px;gap:6px;flex:1}}
+.mid-left,.mid-right{{display:flex;flex-direction:column;gap:4px}}
+.center{{display:flex;flex-direction:column;align-items:center;gap:3px;justify-content:center}}
+.char-img{{width:280px;height:340px;border-radius:8px;object-fit:cover;border:2.5px solid #c4b49a;box-shadow:3px 3px 10px rgba(0,0,0,.12)}}
+.quote{{background:rgba(196,145,90,.08);border:1px dashed rgba(196,145,90,.3);border-radius:4px;padding:4px 8px;text-align:center;width:100%}}
+.quote p{{font-family:'Gaegu',cursive;font-size:12px;color:#8b4513;line-height:1.25}}
...
+.goal-review{{display:grid;grid-template-columns:1fr 1fr;gap:4px;width:100%}}
+.goal-box{{background:rgba(255,255,255,.5);border:1px solid #d4c4a8;border-radius:4px;padding:4px 6px;text-align:center}}
+.goal-box h4{{font-family:'Gaegu',cursive;font-size:11px;color:#8b4513;margin-bottom:2px;display:flex;align-items:center;justify-content:center;gap:3px}}
+.goal-box p{{font-size:9px;color:#5a4f42;line-height:1.35}}
+.step{{display:inline-flex;align-items:center;justify-content:center;width:16px;height:16px;border-radius:50%;background:#c4915a;color:#fff;font-size:9px;font-weight:700;flex-shrink:0}}
+.bottom{{display:grid;grid-template-columns:repeat(4,1fr);gap:5px}}
+.btm-card{{background:rgba(255,255,255,.45);border:1.5px solid #d4c4a8;border-radius:5px;padding:6px 8px}}
...
+.btm-card li{{font-size:9.5px;line-height:1.45;color:#5a4f42;display:flex;align-items:flex-start;gap:3px}}
...
+    <h1>촛불처럼 밝히고,<br>바다처럼 품으며,<br>함께 성장하는 사람</h1>
+    <div class="sub">{I['candle']} 지혜 + {I['water']} 포용 + {I['gear']} 실행 = 만능 멘토 기질</div>
   </div>
...
+        <li>{I['candle']} 은은하게 비추되 꺼지지 않는다</li>
+        <li>{I['water']} 깊이 품되 휘쓸리지 않는다</li>
+        <li>{I['shield']} 함께 성장하되 중심을 잃지 않는다</li>
       </ul>
...
+        <span class="tag">촛불 지혜</span><span class="tag">바다 포용</span><span class="tag">멘토십</span>
+        <span class="tag">직관력</span><span class="tag">분석력</span><span class="tag">실행력</span>
+        <span class="tag">섬세함</span><span class="tag">창의융합</span><span class="tag">공감력</span>
+        <span class="tag">문제해결</span><span class="tag">신뢰감</span><span class="tag">리더십</span>
       </div>
     </div>
   </div>
@@ -273,124 +287,145 @@ body{{width:850px;height:1100px;background:#f5efe6;
 <div class="mid">
   <div class="mid-left">
...
+        <li>{I['candle']} <span>누군가의 성장을 돕는 것에서 가장 큰 기쁨</span></li>
+        <li>{I['shield']} <span>꾸준히 지켜보며 기다려주는 멘토링</span></li>
+        <li>{I['star']} <span>복잡한 것을 쉽게 풀어주는 능력</span></li>
+      </ul>
+    </div>
+    <div class="card">
+      <h3>{I['water']} 대인 관계 스타일</h3>
+      <ul>
+        <li>{I['heart']} <span><b>경청</b> — 말보다 듣기를 먼저</span></li>
+        <li>{I['gear']} <span><b>구조화</b> — 복잡한 이야기도 정리해주는 힘</span></li>
+        <li>{I['candle']} <span><b>진심</b> — 표면이 아닌 근본을 다루는 대화</span></li>
+        <li>{I['shield']} <span><b>인내</b> — 끝까지 들어주는 자세</span></li>
       </ul>
     </div>
   </div>
...
+      <img class="char-img" src="data:image/png;base64,{img_b64}" />
+      <div style="position:absolute;top:5px;left:7px;background:rgba(255,255,255,.8);border-radius:4px;padding:2px 6px;font-size:8px;color:#8b4513;font-family:'Gaegu',cursive;display:flex;align-items:center;gap:3px">{I['candle']} 촛불+바다</div>
     </div>
     <div class="name-area">
...
+        <h4>{I['fire']} 오늘의 목표</h4>
+        <p>매일 촛불처럼 한 가지에 집중<br>방향 발견 → 깊이 파고 → 실행<br>작은 불빛이 모여 큰 방향을 밝힌다</p>
       </div>
       <div class="goal-box">
...
+        <h4>{I['water']} 리뷰 (하루 끝)</h4>
+        <p>→ 발견한 것 매달음 정리<br>→ 못한 점 정직하게<br>→ 다음 실행 체크</p>
+        <p style="font-size:8px;color:#8b4513;font-family:'Gaegu',cursive">작은 개선의 오늘,<br>학습은 내일!</p>
       </div>
     </div>
   </div>
