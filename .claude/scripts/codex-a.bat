...
if "%MODE%"=="analyze" (
  ...
  set "ANALYZE_TASK=%ROOT%\.claude\tasks\task-codex-a-analyze.md"
  if not exist "%ROOT%\.claude\tasks" mkdir "%ROOT%\.claude\tasks" >nul 2>&1
  (
    echo # Task: Project Source Analysis ^(auto-generated by codex-a.bat --analyze^)
    echo.
    echo ## Goal
    ...
    echo 2. Map all source files with their role.
    echo 3. Identify patterns: state management, API calls, auth flow, error handling.
    echo 4. Find improvements: list issues with file+line references.
    echo 5. For .sql / query / mapper files: DO NOT MODIFY. Write suggestions to docs\query-suggestions.md.
    echo 6. Write full analysis to docs\codex-analysis.md.
    echo.
    echo ## Rules
    echo - Relative paths only ^(no hardcoded user paths^).
    echo - Follow all rules in CLAUDE.md.
    echo - Proceed automatically without asking for confirmation.
  ) > "!ANALYZE_TASK!"
  codex exec --dangerously-bypass-approvals-and-sandbox "Read !ANALYZE_TASK! and execute every step in order."
  goto END
)
