...
if exist "%PROJECT_ROOT%\.claude\orca-workers-config.json" (
  for /f "usebackq tokens=*" %%C in (`powershell -NoProfile -Command "try { (Get-Content '%PROJECT_ROOT%\.claude\orca-workers-config.json' -Raw | ConvertFrom-Json).workers.gemini } catch { '' }" 2^>nul`) do (
