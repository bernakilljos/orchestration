...
trends = [
    ...
    ('🛡️ AI Cybersecurity', '$146.5B by 2034', 'Agentic AI 폭증\n신 공격면 (AI Workload)\n2034 6배 성장', GOLD),
    ('🔍 Insider Risk Mgmt', 'UEBA → IRM 통합', 'Gartner 2025 재분류\nSIEM 통합 추세\n부서 핵심 IP 직결', ACCENT),
]
...
add_text(s, 0.5, 6.7, 12.3, 0.4, '⚠ 5,000+ 한국 대기업 모두 영향. AI Risk Lighthouse 가 통합 대응 — 1社 1억 × 5,000 = 5,000억 시장',
...
# 우측: 부서 IP
add_text(s, 6.8, 1.0, 6, 0.4, '🎯 부서 IP (리스크모니터링·행동위험분석)', size=18, bold=True, color=ACCENT)
dept_ip = [
...
add_text(s, 0.7, 6.4, 12, 0.3, '💡 5개 신사업 모두 R&D 자체 개발 없음 (OEM·통합·SaaS·라이선스 모델)',
...
# 좌측: 8 카테고리
add_text(s, 0.3, 1.0, 6, 0.4, '🔍 8 검증 카테고리 (가중 점수)', size=15, bold=True, color=PRIMARY)
cats = [
...
# 좌측: 아키텍처
add_text(s, 0.3, 1.0, 6.2, 0.4, '🏗️ 아키텍처 (Plan-Act-Reflect 루프)', size=15, bold=True, color=PRIMARY)
arch = [
...
# 우측: 적용 사례 + ROI
add_text(s, 6.8, 1.0, 6.2, 0.4, '💡 적용 사례 + ROI', size=15, bold=True, color=ACCENT)
...
add_text(s, 0.3, 1.0, 12.7, 0.4, '🎯 컨셉: 신용평가사(KCB·NICE)처럼 직원·고객 행동위험을 점수화 → 표준 라이선스',
...
# 좌측: 시장 + 기회
add_text(s, 0.3, 1.0, 6.2, 0.4, '📈 시장 + 기회', size=15, bold=True, color=PRIMARY)
add_bg(s, 0.3, 1.5, 6.2, 5.5, WHITE, outline=PRIMARY)

mkt = [
    '🌐 글로벌 시장: $228.58B by 2029 (CAGR 51.5%)',
    '🇰🇷 한국 점유 가능: $20-30억 (SI 독점)',
    '🎯 Gartner 2027: 40%+ 대기업 산업 메타버스 매출',
    '⚡ 핵심 기술: Digital Twin + AR/VR + AI + IoT + 5G',
    '',
    '🏢 ITCEN CORE 자산 활용',
    '  • 디지털트윈 솔루션 (기존 보유)',
...
# 우측: ROI + 의사결정 요청
add_text(s, 6.8, 1.0, 6.2, 0.4, '📊 ROI + 의사결정 요청', size=15, bold=True, color=ACCENT)
...
add_text(s, 6.95, 4.0, 6.0, 0.35, '⚡ 의사결정 요청 사항', size=14, bold=True, color=WHITE)
