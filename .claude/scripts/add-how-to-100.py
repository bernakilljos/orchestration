...
def inject_how(match):
    ...
    h = HOW[num]
    how_html = f'<div class="card-how"><strong>[FIX] 어떻게:</strong> ① {h[0]} -> ② {h[1]} -> ③ {h[2]} <span style="color:#888">(기간 {h[3]})</span></div>'
...
def replace(m):
    ...
    if num not in HOW:
        return full
    h = HOW[num]
    how_html = (f'<div class="card-how"><strong>[FIX] 어떻게:</strong> '
                f'① {h[0]} -> ② {h[1]} -> ③ {h[2]} '
