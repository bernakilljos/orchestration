...
if [ -n "$matches" ]; then
  MSG=$(printf "%b" "🎯 관련 skill/tool 자동 활성 (즉시 실행):${matches}\n\n※ 위 skill/rule 은 지금 즉시 반영·실행 (사용자 명시 확인 없이). Skill tool 로 호출 가능한 것은 바로 Skill 호출.")
  ESC=$(python -c "import json,sys; print(json.dumps(sys.stdin.read()))" <<< "$MSG")
  cat <<EOF
{
  "hookSpecificOutput": {
